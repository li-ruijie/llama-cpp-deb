...
#
# llama-cpp-cuda pins the exact runtime packages it needs rather than pulling
...
# and a silent fall back to CPU. That is the failure NCCL already caused once.
#
...
# package or the C and C++ runtimes, which llama-cpp already depends on.
